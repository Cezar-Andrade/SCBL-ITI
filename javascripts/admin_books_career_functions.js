function generate_report(){
    ...
    rows = [["Rank", "Título", "Autores", "ISBN", "Editorial", "Clasificacion", "Carrera", "Total"]];
    ...
    .then(blob => {
        ...
        link.download = "Libros_mas_consultados_por_carrera_" + document.getElementById("fechainicio").value + "_" + document.getElementById("fechafinal").value + ".xlsx";
        link.click();
    });
}
...
function update_table(inicio, final, value){
    clear_table();
    ...
    .then(data => {
        ...
        for (var i=0; i < deudores_length; i++){
            ...
            temp.appendChild(document.createTextNode(item["Carrera"]));
...
function search_data_combobox(combobox, type_search){
    ...
    .then(data => {
        if (data.status === "user-not-admin" || data.status === "user-not-authenticated"){
            ...
            combobox.addOption({"value": "Todos", "text": "Todos"});
            JSON.parse(data.data).forEach((item) => {
                combobox.addOption({"value": item["Carrera"], "text": item["Carrera"]});
            });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("fechainicio").addEventListener('change', function() {
        ...
        value = carrera_combobox.getValue();
        inicio.max = final.value;
        if (inicio.value !== "" && final.value !== "" && value !== ""){
            update_table(inicio.value, final.value, value);
        }else{
            clear_table();
        }
    });
    ...
    carrera_combobox = $('#carrera').selectize({
        sortField: 'text',
        normalize: true,
    ...
    search_data_combobox(carrera_combobox, "carreras");
});